...
public class TrainingExerciseSeeder implements CommandLineRunner {
    ...
    public void run(String... args) {
        ...
        inserted += ensure("RUNNING",        "跑步",     "🏃", "CARDIO", "CARDIO", "户外跑步 · 长距离心肺训练",  1);
        inserted += ensure("TREADMILL",      "跑步机",   "🏃", "CARDIO", "CARDIO", "室内跑步 · 可控配速",        2);
        inserted += ensure("CYCLING",        "骑行",     "🚴", "CARDIO", "CARDIO", "户外/室内单车",              3);
        inserted += ensure("ROWING",         "划船机",   "🚣", "CARDIO", "CARDIO", "全身性有氧 · 心肺 + 上肢",   4);
        inserted += ensure("ELLIPTICAL",     "椭圆机",   "🏃", "CARDIO", "CARDIO", "低冲击有氧 · 关节友好",      5);
        inserted += ensure("STAIR_CLIMBER",  "爬楼机",   "🪜", "CARDIO", "CARDIO", "下肢有氧 · 臀腿燃脂",        6);
        inserted += ensure("JUMP_ROPE",      "跳绳",     "🪢", "CARDIO", "CARDIO", "高效燃脂 · 协调性训练",      7);
        inserted += ensure("SWIMMING",       "游泳",     "🏊", "CARDIO", "CARDIO", "全身有氧 · 关节零冲击",      8);

        // ============ 胸 CHEST · 14 个 ============
        // ⚠️ emoji 映射策略：参见 RN 端 `builtinTrainingExercises.ts` 头部注释。
        // 原则：命中即用动作 emoji，未命中（如卧推/推举/弯举/屈伸/划船/夹胸/提踵/卷腹...）一律 💪 兜底。
        inserted += ensure("BARBELL_BENCH_PRESS",         "杠铃卧推",       "💪", "CHEST", "BARBELL",    "胸大肌力量训练之王 · 平板凳上水平推举", 10);
        inserted += ensure("INCLINE_BARBELL_BENCH_PRESS", "上斜杠铃卧推",   "💪", "CHEST", "BARBELL",    "针对胸大肌上束 · 凳面 30-45°",        20);
        inserted += ensure("DECLINE_BARBELL_BENCH_PRESS", "下斜杠铃卧推",   "💪", "CHEST", "BARBELL",    "针对胸大肌下束 · 凳面下倾",          30);
        ...
        inserted += ensure("CHEST_FLY_MACHINE",           "蝴蝶机夹胸",     "💪", "CHEST", "MACHINE",    "胸肌孤立 · 轨迹固定适合新手",        80);
        inserted += ensure("CABLE_CROSSOVER",             "龙门架夹胸",     "💪", "CHEST", "CABLE",      "胸肌持续张力 · 末端挤压感强",        90);
        inserted += ensure("PEC_DECK",                    "坐姿夹胸",       "💪", "CHEST", "MACHINE",    "坐姿固定轨迹胸肌孤立训练",          100);
        inserted += ensure("SMITH_BENCH_PRESS",           "史密斯卧推",     "💪", "CHEST", "MACHINE",    "杠铃轨迹固定 · 新手友好",           110);
        ...
        inserted += ensure("BARBELL_ROW",         "杠铃划船",       "💪", "BACK", "BARBELL",    "背阔肌厚度训练 · 俯身姿势",          230);
        inserted += ensure("T_BAR_ROW",           "T 杠划船",       "💪", "BACK", "BARBELL",    "中背训练 · 单边支点稳定",            240);
        ...
        inserted += ensure("LAT_PULLDOWN",        "高位下拉",       "💪", "BACK", "MACHINE",    "背阔宽度训练 · 引体替代品",          270);
        ...
        inserted += ensure("ASSISTED_PULL_UP",    "辅助引体",       "💪", "BACK", "MACHINE",    "机器助力 · 引体进阶训练",            300);
        ...
        // ============ 肩 SHOULDERS · 9 个 ============
        inserted += ensure("OVERHEAD_PRESS",          "站姿杠铃推举", "💪", "SHOULDERS", "BARBELL",  "肩部力量动作之王",            410);
        ...
        inserted += ensure("SMITH_SHOULDER_PRESS",    "史密斯肩推",   "💪", "SHOULDERS", "MACHINE",  "轨迹固定 · 新手友好",        440);
        ...
        inserted += ensure("UPRIGHT_ROW",             "直立划船",     "💪", "SHOULDERS", "BARBELL",  "三角肌中束 + 斜方肌",        490);

        // ============ 二头 BICEPS · 7 个 ============
        inserted += ensure("BARBELL_CURL",          "杠铃弯举",       "💪", "BICEPS", "BARBELL",  "二头基础动作 · 大重量训练",   510);
        inserted += ensure("EZ_BAR_CURL",           "EZ 杠弯举",      "💪", "BICEPS", "BARBELL",  "弯曲握把 · 手腕更舒适",       520);
        ...
        inserted += ensure("PREACHER_CURL",         "牧师凳弯举",     "💪", "BICEPS", "MACHINE",  "孤立训练 · 杜绝借力",         560);
        inserted += ensure("CABLE_CURL",            "绳索弯举",       "💪", "BICEPS", "CABLE",    "持续张力 · 全程顶峰收缩",     570);

        // ============ 三头 TRICEPS · 8 个 ============
        inserted += ensure("CLOSE_GRIP_BENCH_PRESS",      "窄距卧推",       "💪", "TRICEPS", "BARBELL",    "三头复合动作 · 兼顾胸下束", 610);
        inserted += ensure("SKULL_CRUSHER",               "颅骨破碎者",     "💪", "TRICEPS", "BARBELL",    "三头孤立 · 仰卧屈伸",       620);
        ...
        inserted += ensure("GOBLET_SQUAT",           "高脚杯深蹲",     "🏋️", "LEGS", "DUMBBELL",   "哑铃抱胸 · 新手友好",            730);
        ...
        inserted += ensure("LEG_PRESS",              "倒蹬机",         "💪", "LEGS", "MACHINE",    "腿部大重量训练 · 安全可控",      760);
        inserted += ensure("LEG_EXTENSION",          "腿屈伸",         "💪", "LEGS", "MACHINE",    "股四头肌孤立训练",                770);
        inserted += ensure("LEG_CURL",               "腿弯举",         "💪", "LEGS", "MACHINE",    "腘绳肌孤立训练 · 俯卧/坐姿",    780);
        ...
        // ============ 小腿 CALVES · 3 个 ============
        inserted += ensure("STANDING_CALF_RAISE", "站姿提踵", "💪", "CALVES", "MACHINE",  "腓肠肌训练 · 直腿提踵",      910);
        inserted += ensure("SEATED_CALF_RAISE",   "坐姿提踵", "💪", "CALVES", "MACHINE",  "比目鱼肌训练 · 屈膝提踵",    920);
